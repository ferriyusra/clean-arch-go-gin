...
func TestAdminMuxMetricsBodyIsPrometheusText(t *testing.T) {
	...
	metrics.Middleware() // no requests yet; the families still have to be exported
...
func TestNewAdminServer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			testutil.True(t, server.WriteTimeout >= tt.wantMinWrite, "write timeout")
		})
	}
}
...
func TestNewMetricsWithRegistryIsIsolated(t *testing.T) {
	...
	observability.NewMetricsWithRegistry(first)
	...
	// Both are empty until a request is recorded, and neither has picked up the
	// other's instruments.
	testutil.Equal(t, len(firstFamilies), 0, "first registry starts empty")
	testutil.Equal(t, len(secondFamilies), 0, "second registry starts empty")
}
